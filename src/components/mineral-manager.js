...
export const useMineralManager = () => {
  const [mineralManagerIsVisible, setMineralManagerVisibility] = useState(false);

  const openMineralManager = () => setMineralManagerVisibility(true);
  const closeMineralManager = () => setMineralManagerVisibility(false);

  const MineralManager = () => {
    const { userMinerals, setMineralValue, resetMineralValue } = useMinerals();

    if (!mineralManagerIsVisible) {
      return null;
    }
    ...
    return (
      <div className="fixed z-50 flex justify-center items-center inset-0">
        <div className="fixed z-0 w-full h-full bg-gray-700 opacity-75" onClick={closeMineralManager}></div>
        <div className="relative z-10 bg-white rounded-md overflow-hidden shadow-md w-3/4 md:w-1/2 lg:w-1/3 max-h-screen overflow-y-auto">
          ...
          <div className="flex justify-end p-4">
            <button className="bg-blue-600 text-blue-100 py-1 px-2 rounded" onClick={closeMineralManager}>Done</button>
          </div>
        </div>
      </div>
    );
  ...
  return { MineralManager, openMineralManager, closeMineralManager };
}
